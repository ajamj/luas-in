...
class WiFiMonitorServer(ctk.CTk):
    def __init__(self):
        ...
        # Inisialisasi core
        self.core = LuasinCore(port=5000, password="1234")
    ...
    def init_ui(self):
        ...
        ctk.CTkLabel(self.config_frame, text="Port:").grid(
        ...
        ctk.CTkLabel(self.config_frame, text="Password:").grid(
        ...
        ctk.CTkLabel(self.config_frame, text="Target Monitor:").grid(
            row=4, column=0, padx=10, pady=5, sticky="w"
        )
        self.monitor_combo = ctk.CTkComboBox(
            self.config_frame, values=self.get_monitors()
        )
        ...
        self.lbl_ext_info = ctk.CTkLabel(
            self.config_frame,
            text="💡 For Extended Mode, ensure Windows detects a 2nd display.",
            font=ctk.CTkFont(size=10, slant="italic"),
            text_color="gray",
        )
        self.lbl_ext_info.grid(
            row=5, column=0, columnspan=2, padx=10, pady=(0, 5), sticky="w"
        )
        ...
        self.cb_sync = ctk.CTkCheckBox(
            self.features_frame, text="Clipboard Sync", variable=self.cb_sync_var
        )
        self.cb_sync.grid(row=0, column=0, padx=10, pady=10)

        self.btn_send_file = ctk.CTkButton(
            self.features_frame,
            text="Send File",
        ...
        self.lbl_status = ctk.CTkLabel(
            self.status_frame, text="Status: Stopped", text_color="#e74c3c"
        )
        ...
        # Logs
        ctk.CTkLabel(self, text="System Logs").grid(
    ...
    def update_status_ui(self, running):
        if running:
            ...
            self.btn_toggle.configure(
                text="Stop Server", fg_color="#e74c3c", hover_color="#c0392b"
            )
        else:
            self.lbl_status.configure(text="Status: Stopped", text_color="#e74c3c")
            self.btn_toggle.configure(
                text="Start Server", fg_color="#2ecc71", hover_color="#27ae60"
            )
    ...
    def send_packet(self, ptype, data):
        ...
        if disconnected:
            self.after(
                0,
                lambda: self.lbl_clients.configure(
                    text=f"Connected Clients: {len(self.clients)}"
                ),
            )
            ...
            try:
                ...
                if ptype == TYPE_CLIPBOARD:
                    ...
                    if client_socket in self.incoming_files:
                        ...
                        self.add_log(
                            f"File '{self.incoming_files[client_socket]['name']}' saved."
                        )
                        del self.incoming_files[client_socket]
                elif ptype == TYPE_INPUT:
                    # Format data input: [type:1][x:2][y:2][button:1] (sederhana)
                    input_type = data[0]
